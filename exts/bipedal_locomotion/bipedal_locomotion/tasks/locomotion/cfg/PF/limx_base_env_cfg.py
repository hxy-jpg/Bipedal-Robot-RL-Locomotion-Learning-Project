...
class CommandCfg(BaseCommandsCfg):
    ...
    def __post_init__(self):
        ...
        self.base_velocity.heading_command = False        # 启用航向命令 / Enable heading commands
        ...
        self.base_velocity.rel_standing_envs = 0.5       # 站立环境比例 / Standing environments ratio
        ...
        self.base_velocity.ranges = mdp.UniformVelocityCommandCfg.Ranges(
            ...
            lin_vel_y=(-0.6, 0.6),      # 横向速度范围 [m/s] / Lateral velocity range [m/s]
...
class ActionsCfg:
    ...
    joint_pos = mdp.JointPositionActionCfg(
        ...
        scale=0.5,              # 动作缩放因子 / Action scaling factor
...
class EventsCfg:
    ...
    push_robot = EventTerm(
        ...
        interval_range_s=(2.5, 5.0),               # 间隔时间范围 / Interval time range
        params={
            "asset_cfg": SceneEntityCfg("robot", body_names="base_Link"),
            # 力的范围 [N] / Force range [N]
            "force_range": {
                "x": (-100.0, 100.0), "y": (-100.0, 100.0), "z": (-0.0, 0.0),
            },
            # 力矩范围 [N⋅m] / Torque range [N⋅m]
            "torque_range": {"x": (-0.0, 0.0), "y": (-0.0, 0.0), "z": (-0.0, 0.0)},
            "probability": 0.7,                   # 发生概率 / Occurrence probability
        },
        is_global_time=False,
        min_step_count_between_reset=0,
    )
...
class RewardsCfg:
    ...
    pen_base_height = RewTerm(
        ...
        weight=0.0,                               # 负权重表示惩罚 / Negative weight indicates penalty
    )
